...
export default function AdminIncidentDetailPage() {
  ...
  const { data: incident, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (res.error)
        throw new Error(
          (res.error.value as any)?.message || "Failed to load incident",
        );
      ...
      data.witnesses = Array.isArray(data.witnesses)
        ? data.witnesses
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      if (res.error)
        throw new Error(
          (res.error.value as any)?.message || "Failed to update",
        );
      return res.data;
    },
    onSuccess: () => {
    ...
    <div className="space-y-6 max-w-6xl">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              ...
              {incident.created_by_user &&
                ` by ${incident.created_by_user.username}`}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      {incident.user_unable_to_respond && (
        <Alert className="border-red-300 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>User Unable to Respond</strong> — User is injured,
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {incident.official_photos &&
                Array.isArray(incident.official_photos) &&
                incident.official_photos.length > 0 && (
                  <div>
            ...
            <Card className="border-green-200">
              <CardHeader className="pb-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Submitted{" "}
                  {incident.user_responded_at &&
                    formatDistanceToNow(new Date(incident.user_responded_at), {
                      addSuffix: true,
                    })}
                </p>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                {incident.user_photos &&
                  Array.isArray(incident.user_photos) &&
                  incident.user_photos.length > 0 && (
                    <div>
                      <p className="text-xs font-medium text-muted-foreground mb-2">
              ...
              <CardContent className="flex items-center gap-3 py-4">
                ...
                <div>
                  <p className="text-sm font-medium text-yellow-900">
                    Awaiting User Statement
                  </p>
                  <p className="text-xs text-yellow-700">
                    The user has not yet provided their account of the incident.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base">
                  Witness Statements ({incident.witnesses.length})
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {incident.witnesses.map((w, i) => (
                  <div
                    key={i}
                    className="border-l-2 border-muted-foreground/20 pl-4 space-y-1"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">{w.name}</span>
                      <span className="text-[10px] text-muted-foreground">
                        {format(new Date(w.added_at), "PPp")}
                      </span>
                    </div>
                    {w.contact && (
                      <p className="text-xs text-muted-foreground">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center gap-3">
                <Avatar className="h-9 w-9 border border-gray-200 shadow-sm">
                  <AvatarImage
                    src={
                      incident.booking_user?.avatar_url
                        ? incident.booking_user.avatar_url.startsWith("http")
                          ? incident.booking_user.avatar_url
                          : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/api/v1/uploads/${incident.booking_user.avatar_url}`
                        : ""
                    }
                  ...
                  <AvatarFallback className="bg-gray-100 text-gray-600 font-bold">
                    {incident.booking_user?.username?.charAt(0).toUpperCase() ||
                      "U"}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <p className="text-sm font-medium">
                    {incident.booking_user?.username || "Unknown"}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {incident.booking_user?.email}
                  </p>
                </div>
              </div>
              <Separator />
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Select
                  value={status}
                  onValueChange={(v: string) => setStatus(v as IncidentStatus)}
                >
